...
def validate(model, algorithm, validation_input, validation_output, std_y_train, mean_y_train):
    with torch.no_grad():
        x = validation_input
        y = validation_output
#        print("test check", x.size())
        val_pred_cat = []
        for index in range(len(x)):
#            print("index check:", index, x.size())
            x = validation_input[index]
            y = validation_output[index]
            x, y = x.to(algorithm.device), y.to(algorithm.device)
            val_pred_diff, val_pred_SNN, val_pred_CNN = model(x)
...
#        print("check size:", val_pred_cat.size())
