...
if __name__ == '__main__':  # multi-processing protection
    ...
    val_batch_size = 1000 #batchsize for evaluation
    train_batch_size = 5000 #batchsize for train
    ...
    for index_model in index_models:
        for rep_index in range(amount_repeat):
            NN_dataset_name = 'data/lotka_volterra.mat'       
            for data_norm_sign in data_norm_sign_list:
                ...
                if data_norm_sign == 'original':
                    ...
                    estimation_input_pro, estimation_output_pro, validation_input_pro, std_y_train, mean_y_train, mean_X_train, std_X_train = \
                        lib.clean_data(estimation_input_pro, estimation_output_pro, validation_input_pro,
                                       onoff_normalize_data)   														
                input_features = estimation_input_pro.size(1)										
                output_features = estimation_output_pro.size(1)
                ...
                for cycle in range(cycles):
                    for epoch in range(epochs):
                        ...
                        epoch_record += 1
                        # train with mask
                        pred, pred_SNN, target, train_loss, hessian_auto = lib.fine_train_node(model, algorithm, optimizer, estimation_input_pro,
                        ...
                        if torch.sum(torch.abs(pred_SNN))==0:
                            print("here")
                            print('sign 1: DISCONNECTED SNN')
                            print('sign 1 epoch:', count_epoch, torch.sum(pred_SNN))
