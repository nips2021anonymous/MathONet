...
class CONV_stencil(torch.nn.Module):
    def __init__(self,  save_device=torch.device('cpu')): 
        super(CONV_stencil, self).__init__()
        self.save_device = save_device        
        self.conv1 = torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(3,1),  bias = False) 
        self.switch_stencil_1 = nn.Parameter(torch.ones(1).to(save_device))            
    ...
    def forward(self, x):              
        ...
        x1.append(x1[0])
        x1.insert(0, x1[-2])        
        x1 = torch.tensor(x1).squeeze().unsqueeze(1).to(self.save_device)         
        x1 = x1.unsqueeze(0).unsqueeze(0) # turn input: x3 from 2-D form into 4-D form                        
        x_opt_1 = self.switch_stencil_1*self.conv1(x1)                       
        x = x_opt_1                             
        return x
...
class MathONet(nn.Module):
    def __init__(self, MathONet_layers, unary_names_lists, save_device=torch.device('cuda')):
        ...
        self.ConvNet = CONV_stencil(save_device=save_device)
    ...
    def forward(self, x):     
        ...
        x2 = torch.tensor([0.]).to(self.save_device)        
        if len(self.MathONet_layers) > 1:
            x1 = copy.deepcopy(x)                      
